...
def run_bias_correction(city, variable, start_time, end_time):
    ...
    try:
        ...
        if not os.path.exists(file2_path):
            file2_path = f'dataset/Default{variable}.csv'
        ...
        yyq_train = yqt.transform(y_train.reshape(-1, 1)).ravel() 
        ...
        predict_df = pd.DataFrame(columns=["Model", "Fold", "Predictions", "True Values"])
        ...
        kf = KFold(n_splits=5, shuffle=True, random_state=42)  # cv
        for model_name, model in models.items():
            ...
            results['descriptions']['model_scores'][model_name] = np.mean(cv_scores)  
 
        if end_year1 < start_year1:
            raise ValueError("End year should not be greater than 2099 and should not be less than the start year.")

        if start_year1 > end_year1:
            raise ValueError("Start year should not be earlier than 2017.")
        ...
        csv_file_path = 'static/reports/LR_RF_.csv'
        predict_df.to_csv(csv_file_path, index=False)
